...
class NumberPlateProcessor:
    ...
    def obtain_text(
        ...
        obtained_number_plate = NumberPlateProcessor.number_plate_filtering(
            generated_text=generated_text,
            all_patch_coodinates=all_patch_coodinates,
        )

        if obtained_number_plate != "":
            return obtained_number_plate

        # Give a final pass
        # To find double row number plate
        all_patch_coodinates = self.sorted_highest_area(all_patch_coodinates)

        # Split the images into two parts
        x1, y1, x2, y2 = all_patch_coodinates[0]
        cropped_image = single_image[..., y1:y2, x1:x2] * 255
        all_patch_coodinates = [
            [x1 - 10, x2 + 10, y1 - 10, cropped_image.size(1) // 2 + 10],
            [x1 - 10, x2 + 10, cropped_image.size(1) // 2 - 10, y2 + 10],
        ]

        if debug:
            print(cropped_image.size(1) / cropped_image.size(2))

        pixel_values = self.text_processor(
            [
                cropped_image[::, : cropped_image.size(1) // 2, ::],
                cropped_image[::, cropped_image.size(1) // 2 :, ::],
            ],
            return_tensors="pt",
        ).pixel_values.to(device)
    ...
    @staticmethod
    def number_plate_filtering(
        generated_text: List[Any],
        all_patch_coodinates: List[Any],
        ...
        for idx, generated_text in enumerate(generated_text):
            ...
            if enable_area_check:
                x1, y1, x2, y2 = all_patch_coodinates[idx]
                if NumberPlateProcessor.area(all_patch_coodinates[idx]) < 700:
                    continue
                if ((y2 - y1) / (x2 - x1)) > 1:
                    continue

            if re.match(r"^[a-z][a-z]?[a-z4]?[a-z]?$", generated_text):
                ...
                r"^([a-z][a-z]?[a-z4]?[a-z]?)([0-9]{1,4})([a-z]{1,4})?$", generated_text
            ):
    ...
    @staticmethod
    def sorted_highest_area(all_patch_coodinates: List[Any]):
        unsorted_pair = []
        for i in range(len(all_patch_coodinates)):
            area = NumberPlateProcessor.area(all_patch_coodinates[i])
            unsorted_pair.append((area, i))
        sorted_index = sorted(unsorted_pair, key=lambda x: x[0])
        return [all_patch_coodinates[i] for area, i in sorted_index]
